# modules/PXSearchOrders.py
# ------------------------------------------------------------
# Projeto Jocasta — PXSearchOrders
# Histórico / Consulta de rolos exportados (SQLite)
#
# UI simplificada:
# - Sem filtro/coluna "Modo"
# - Mantém "Eventos" como contador
# - Aba "Eventos" lista: data/hora | tipo | resumo do payload
# - Sem aba "Payload" (payload continua salvo no DB)
#
# Atualização:
# - Campo "Pedido contém": filtra rolos que tenham ao menos 1 order/documento com esse texto
# ------------------------------------------------------------
...
def _payload_summary(payload_json: str) -> str:
    ...
    try:
        ...
        if "reexport" in p:
            parts.append(f"reexport={bool(p.get('reexport'))}")

        if "orders_count" in p:
            parts.append(f"orders={p.get('orders_count')}")
...
def _safe_int(s: str, default: int = 200) -> int:
    try:
        v = int(str(s).strip())
...
class PXSearchOrdersUI(ttk.Frame):
    def __init__(self, parent):
        ...
        ttk.Button(filtros, text="Recarregar", command=self.reload).grid(row=0, column=8, sticky="w", padx=(0, 8), pady=8)
        ttk.Button(filtros, text="Limpar filtros", command=self.clear_filters).grid(row=0, column=9, sticky="w", padx=(0, 10), pady=8)
    ...
    def reload(self) -> None:
        limit = _safe_int(self.var_limit.get(), default=300)
        ...
        try:
            ...
            # fallback se sua versão antiga de list_rolls não tiver todos params
            try:
                rows = list_rolls(limit=limit, machine=machine, name_like=name_like)  # type: ignore
            except Exception as e:
                messagebox.showerror("PXSearchOrders", f"Falha ao carregar rolos.\n\n{type(e).__name__}: {e}")
                return
        except Exception as e:
            messagebox.showerror("PXSearchOrders", f"Falha ao carregar rolos.\n\n{type(e).__name__}: {e}")
            return
        ...
        for r in rows:
            try:
                rid = int(r.get("id", 0))
                roll_name = str(r.get("roll_name", ""))
                mach = str(r.get("machine", ""))
                created = str(r.get("created_at", ""))
                total_m = float(r.get("total_m", 0.0) or 0.0)
                orders_count = int(r.get("orders_count", 0) or 0)
                events_count = int(r.get("events_count", 0) or 0)
            except Exception:
                continue
            ...
            self.tree_rolls.insert(
                ...
                "end",
                iid=str(rid),
                values=(rid, roll_name, mach, created, _fmt_m(total_m), orders_count, events_count),
            )
            count += 1
    ...
    def _load_summary(self, roll_id: int) -> None:
        try:
            s = get_roll_summary(int(roll_id))
        except Exception as e:
        ...
        lines = []
        lines.append(f"Rolo: {s.get('roll_name', '')}")
        lines.append(f"Máquina: {s.get('machine', '')}")
        lines.append(f"Criado: {s.get('created_at', '')} | Versão: {s.get('app_version', '')}")
        lines.append("")
        lines.append(f"Pedidos: {s.get('orders_count', 0)}")
        lines.append(f"Total (m): {_fmt_m(s.get('total_m', 0))}")
        lines.append(f"EndTime: {s.get('oldest_end', '')} → {s.get('newest_end', '')}")
        lines.append("")
        lines.append("Tecidos (por metragem):")

        fabrics = s.get("fabrics", []) or []
        for f in fabrics:
            fab = f.get("fabric", "DESCONHECIDO")
            m = _fmt_m(f.get("m", 0))
            n = f.get("n", 0)
            lines.append(f"  - {fab}: {m} m ({n} pedidos)")
    ...
    def _load_orders(self, roll_id: int) -> None:
        self.tree_orders.delete(*self.tree_orders.get_children())
        try:
            orders = get_roll_orders(int(roll_id))
        except Exception as e:
            self.status.configure(text=f"Falha ao carregar pedidos: {type(e).__name__}")
            return

        for o in orders:
            end_time = str(o.get("end_time", "") or "")
            document = str(o.get("document", "") or "")
            fabric = str(o.get("fabric", "") or "")
            height_mm = o.get("height_mm", 0) or 0
            vpos_mm = o.get("vpos_mm", 0) or 0
            real_m = o.get("real_m", 0) or 0
            source_path = str(o.get("source_path", "") or "")
            try:
    ...
    def _load_events(self, roll_id: int) -> None:
        self.tree_events.delete(*self.tree_events.get_children())
        try:
            events = get_roll_events(int(roll_id))
        except Exception as e:
        ...
        for ev in events:
            ...
            summary = _payload_summary(payload_json)
            self.tree_events.insert("", "end", values=(created_at, event_type, summary))
